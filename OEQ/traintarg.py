...
#save losses
with open(project_path+'loss_mtl_oeq.pkl', "wb") as tf:
    pickle.dump(history_mtl,tf)

torch.save({'encoder_mtl':encoder_mtl, 'decoder_mtl':decoder_mtl}, project_path+'model_mtl_oeq.pth') #save model

state_dict = torch.load(project_path+'model_mtl_oeq.pth',map_location=torch.device(device.type))
encoder_mtl = state_dict['encoder_mtl'].to(device)
decoder_mtl = state_dict['decoder_mtl'].to(device)
...
for i in range(1,6):
  ...
  en=encoder_mtl(train_dataset[20*i][0].unsqueeze(0).to(device))
  segmented_image=np.array(np.array(transforms.ToPILImage(mode='L')(decoder_mtl(en)[0]))>125,dtype='uint8')
  '''
...
for i in range(len(test_dataset)):
  ...
  en=encoder_mtl(test_dataset[i][0].unsqueeze(0).to(device))
  segmented_image=np.array(np.array(transforms.ToPILImage(mode='L')(decoder_mtl(en)[0]))>125,dtype='uint8')
...
for i in range(len(train_dataset)):
  ...
  en=encoder_mtl(train_dataset[i][0].unsqueeze(0).to(device))
  segmented_image=np.array(np.array(transforms.ToPILImage(mode='L')(decoder_mtl(en)[0]))>125,dtype='uint8')
  acc,ioc=accuracy_seg(segmented_image,target_image)
