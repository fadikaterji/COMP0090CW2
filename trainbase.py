...
for i in range(1,6):
  ...
  en=encoder_sgl(train_dataset[20*i][0].unsqueeze(0).to(device))
  segmented_image=np.array(np.array(transforms.ToPILImage(mode='L')(decoder_sgl(en)[0]))>125,dtype='uint8')
  '''
...
for i in range(len(test_dataset)):
  ...
  en=encoder_sgl(test_dataset[i][0].unsqueeze(0).to(device))
  segmented_image=np.array(np.array(transforms.ToPILImage(mode='L')(decoder_sgl(en)[0]))>125,dtype='uint8')
...
for i in range(len(train_dataset)):
  ...
  en=encoder_sgl(train_dataset[i][0].unsqueeze(0).to(device))
  segmented_image=np.array(np.array(transforms.ToPILImage(mode='L')(decoder_sgl(en)[0]))>125,dtype='uint8')
  acc,ioc=accuracy_seg(segmented_image,target_image)
